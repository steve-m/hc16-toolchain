; $Id: float.s,v 1.2 2008/08/16 05:42:17 querbach Exp $
...
fp_push_uint
	...
	jsr	ulong_float	; convert long on stack to float
	rts


;----------------------------------------------------------------------------
;
;	fp_push_ulong	- push long unsigned integer to float stack
;
;	input:	d points to 4-byte unsigned integer
;	action:	open space on stack
;		put long unsigned integer on stack, set scale and normalize
;	output:	top stack entry holds number in stack format
;
fp_push_ulong
	pshd			; protect registers
	...
	jsr	ulong_float	; convert unsigned long on stack to float
...
;----------------------------------------------------------------------------
;
;	ulong_float	- convert unsigned long integer to float
;
;	input:	mantissa of top float-stack entry holds 4-byte unsigned int
...
ulong_float
	...
	clr	sgn,x		; mark as positive

; convert to float
	clr	exp,x		; kill any exponent
...
;
fp_pop_uint
	jsr	float_ulong	; convert top of stack to long integer
...
;----------------------------------------------------------------------------
;
;	fp_pop_ulong	- pop stack as long unsigned integer
;
;	input:	x points to float on top of stack
;		d points to target for result
;	action:	convert floating point value to long unsigned integer
;	output:	d points to 4-byte unsigned integer
;		  0 if underrange, FFFFFFFF if overrange
;		top stack entry deleted
;
fp_pop_ulong
	pshd			; protect registers
	...
	jsr	float_ulong	; convert accumulator to unsigned long int
...
;----------------------------------------------------------------------------
;
;	float_ulong	- convert floating-point number to unsigned long
;
;	input:	x points to floating-point number on top of float-stack
;	action:	determine scale and denormalize
;	output:	mantissa of top float-stack entry holds 4-byte unsigned int
;
float_ulong
	pshd			; protect register

; if too small, force zero
	ldab	exp,x		; get exponent
	subb	#exp_bias	; does number have an integer part?
	bhs	flt_u1		; yes, go on

	clrd			; load zero
	bra	flt_u3		; go on

; if negative, force zero
flt_u1	tst	sgn,x		; number positive?
	bpl	flt_u2		; yes, go on

	clrd			; load zero
	bra	flt_u3		; go on

; if too large, force maximum value
flt_u2	subb	#31		; is number small enough to be made a ulong?
	bcs	flt_u4		; yes, go on

	ldd	#0FFFFh		; load maximum long unsigned integer

flt_u3	std	man0,x		; force mantissa value
	std	man2,x
	bra	flt_u5		; exit

; shift mantissa to proper position to represent integer
flt_u4	negb			; make number of shifts required
	jsr	denorm_top	; de-normalize number

; done, exit
flt_u5	puld			; recover register
	rts
...
fix_errors
	...
	ldy	#error_tbl	; point to table of results to substitute
	ldab	#error_siz	; size of each entry
fix_e1	cmpa	0,y		; found error?
	beq	fix_e2		; yes, go on

	aby			; next entry
	cpy	#error_end	; done all?
	blo	fix_e1		; no, loop
	
fix_e2	ldd	1,y		; get sign byte and exponent
	std	sgn,x
	ldd	3,y		; get high word of mantissa
	std	man0,x	
	ldd	5,y		; get low word of mantissa
	std	man2,x

;	pshx			; protect register
;	xgxy			; point to message
;	addx	#err_val_siz
;	xjsr	ebug_puts	; show through test interface
;	pulx			; recover register

fix_e3	puly			; recover registers
	puld
	rts


error_tbl
	db	err_under,     00h,000h, 000h,000h,000h,000h ; zero
	db	' PU',0

	db	err_under+80h, 00h,000h, 000h,000h,000h,000h ; zero
	db	' NU',0

	db	err_over,      00h,0FEh, 0FFh,0FFh,0FFh,0FFh ; positive maximum
	db	' PO',0

	db	err_over+80h,  80h,0FEh, 0FFh,0FFh,0FFh,0FFh ; negative maximum
	db	' NO',0

	db	err_div0,      00h,0FEh, 0FFh,0FFh,0FFh,0FFh ; positive maximum
	db	' PZ',0

	db	err_div0+80h,  80h,0FEh, 0FFh,0FFh,0FFh,0FFh ; negative maximum
	db	' NZ',0

error_end
	db	0, 	       00h,000h, 000h,000h,000h,000h  ; unknown error
err_val_siz	equ	$-error_end
	db	' ??',0
error_siz	equ	$-error_end			; size of each entry
