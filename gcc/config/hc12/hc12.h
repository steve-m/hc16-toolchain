/* $Id: hc12.h,v 1.13 2011/05/01 23:12:47 querbach Exp $ */
/* Definitions of target machine for GNU compiler.  Motorola 68HC12 version.

   Copyright (C) 2000-2002 Real Time Systems Inc.
...
/* FIXME doesn't work for cygwin32
#define abort()								\
(               	        					\
  (									\
    (__INCLUDE_LEVEL__) ? 						\
...
#define CPP_SPEC							\
  "									\
    ...
    %{fomit-frame-pointer:  -D__OMIT_FRAME_POINTER__ }			\
  "
...
#define CPP_PREDEFINES "-Dmc68hc12 					\
			-DMC68HC12					\
			-Dhc12						\
			-DHC12"
...
#define TARGET_VERSION fprintf (stderr, " (MC68HC12, GAS syntax)");
...
#define REG_ALLOC_ORDER { DREG, YREG, XREG, SREG, APREG }
...
{									\
  0,									\
			   ...
			   1 << XREG,					\
				       1 << YREG | 1 << DREG,		\
			   1 << XREG | 1 << YREG,			\
			   1 << XREG | 1 << YREG | 1 << DREG,		\
  1 << APREG | 1 << SREG | 1 << XREG | 1 << YREG,			\
  1 << APREG | 1 << SREG | 1 << XREG | 1 << YREG | 1 << DREG		\
}
...
#define BASE_REG_CLASS							\
  (									\
    flag_omit_frame_pointer ? YX_REGS : Y_REGS				\
  )
...
#define REG_CLASS_FROM_LETTER(C)					\
  (									\
    ...
    ((C) == 'x') ? X_REGS   :						\
    ((C) == 'D') ? DY_REGS  :						\
    ((C) == 'C') ? (flag_omit_frame_pointer ? DYX_REGS : DY_REGS) :	\
    ((C) == 'B') ? (flag_omit_frame_pointer ? YX_REGS  : Y_REGS)  :	\
    ((C) == 'A') ? (flag_omit_frame_pointer ? PTR_REGS : Y_REGS)  :	\
		   NO_REGS						\
  )     								\
...
#define EXTRA_CONSTRAINT(VALUE, C)                                      \
  (                                                                     \
    ...
    && GET_CODE(VALUE) == MEM                                           \
    && (                                                                \
	   CONSTANT_ADDRESS_P(XEXP(VALUE, 0))                           \
	|| LEGIT_BASE_ADDR(XEXP(VALUE, 0))                              \
	|| LEGIT_BASE_SHORT_OFFSET_ADDR(XEXP(VALUE, 0))                 \
	|| LEGIT_BASE_INDEX_ADDR(XEXP(VALUE, 0))                        \
	|| LEGIT_MODIFIED_BASE_ADDR(XEXP(VALUE, 0))                     \
       )                                                                \
  )
...
#define FRAME_POINTER_REGNUM XREG
...
#define RETURN_IN_MEMORY(TYPE)						\
  (									\
    GET_MODE_SIZE(TYPE_MODE(TYPE)) > 2					\
  )
...
{									\
  int args = current_function_args_size;				\
  int locs = SIZE + current_function_outgoing_args_size;		\
									\
  if (frame_pointer_needed)						\
  {									\
    fprintf (STREAM, "\tpshx\n");					\
    fprintf (STREAM, "\ttfr\ts,x\n");					\
  }									\
  if (locs)								\
    fprintf (STREAM, "\tleas\t-%d,s\n", locs);				\
									\
  fprintf (STREAM, "\n");						\
}
...
{                                                                       \
  ...
  if (LEGIT_BASE_OFFSET_ADDR(Addr))                                     \
    goto LABEL;                                                         \
									\
  if (LEGIT_MODIFIED_BASE_ADDR(Addr))                                   \
    goto LABEL;                                                         \
									\
  if (MODE != QImode && LEGIT_BASE_INDEX_ADDR(Addr))                    \
    goto LABEL;                                                         \
									\
  if (MODE != QImode && LEGIT_INDIRECT_ADDR(Addr))                      \
    goto LABEL;                                                         \
									\
  if (MODE == QImode && LEGIT_INDIRECT_NONBASEINDEX_ADDR(Addr))         \
    goto LABEL;                                                         \
}                                                                       \
...
#define ASM_DECLARE_FUNCTION_NAME(STREAM, NAME, DECL)			\
  do {									\
    ...
    {                   						\
      ...
      {									\
	fprintf(STREAM, "\tcall\tL");  ASM_OUTPUT_LABEL(STREAM, NAME);	\
	fprintf(STREAM, "\trts\n");					\
									\
	text_section();							\
...
{                                                                       \
  ...
  {                                                                     \
    ...
    fprintf(STREAM, ",%s", reg_names[REGNO(XEXP(Addr, 0))]);            \
									...
									\
  else if (LEGIT_MODIFIED_BASE_ADDR(Addr))                              \
  {                                                                     \
    if (*HC1x_offset)                                                   \
      fprintf(STREAM, "illegal: %s", HC1x_offset);                      \
									\
    fprintf(STREAM, "%d", GET_MODE_SIZE(hc1x_mem_mode));                \
									\
	...
	break;                                                          \
									...
									\
  else if (LEGIT_BASE_INDEX_ADDR(Addr))                                 \
  {                                                                     \
    if (*HC1x_offset)                                                   \
      fprintf(STREAM, "illegal: %s", HC1x_offset);                      \
									\
    if (REGNO(XEXP(Addr, 0)) == DREG)                                   \
      fprintf(STREAM, "%s,%s", reg_names[REGNO(XEXP(Addr, 0))],         \
			       reg_names[REGNO(XEXP(Addr, 1))]);        \
    else if (REGNO(XEXP(Addr, 1)) == DREG)                              \
      fprintf(STREAM, "%s,%s", reg_names[REGNO(XEXP(Addr, 1))],         \
			       reg_names[REGNO(XEXP(Addr, 0))]);        \
    else                                                                \
    {									\
      fprintf(STREAM, "bad base-index address");                        \
      print_rtl(STREAM, ADDR);						\
    }									\
  }                                                                     \
									\
  else if (LEGIT_INDIRECT_ADDR(Addr))                                   \
  {                                                                     \
    Addr = XEXP(Addr, 0);                                               \
									\
    fprintf(STREAM, "[%s", HC1x_offset);                                \
    ...
    {                                                                   \
      fprintf(STREAM, "bad indirect address");                          \
      print_rtl(STREAM, ADDR);						\
    }                                                                   \
									\
    fprintf(STREAM, "]");                                               \
...
/* A 'C' statement to output to STREAM a calculation of number of bytes
   between the start of the function described by DECL and LABEL.  */
                         
#define ASM_OUTPUT_FUNCTION_SCOPE(STREAM, LABEL, DECL)			\
  do {									\
    assemble_name (STREAM, LABEL);					\
    fputs ("-", STREAM);						\
    if (TARGET_MEDIUM)							\
      fputs ("L", STREAM);						\
    assemble_name (STREAM, XSTR (XEXP (DECL_RTL (DECL), 0), 0));	\
  } while (0)
...
/* Required for debugging with -gdwarf. */

#define SECTION_ASM_OP		"section"
#define UNALIGNED_SHORT_ASM_OP	"dc.w"
#define UNALIGNED_INT_ASM_OP	"dc.l"
#define UNALIGNED_OFFSET_ASM_OP	UNALIGNED_INT_ASM_OP
#define UNALIGNED_WORD_ASM_OP	UNALIGNED_INT_ASM_OP
