...
md_begin ()
{
  ...
  for (opcode = hc16_opcodes; opcode->name; opcode++)
  {
...
//---------------------------------------------------------------------------
//
//	Parse an signed byte from a string.
//
...
//---------------------------------------------------------------------------
//
//	Parse multiple register mask for push instruction
//
...
//---------------------------------------------------------------------------
//
//	Parse multiple register mask for pulm instruction
//
...
)
{
  ...
  struct hc16_opcode* opcode;	// opcode table entry
  char* name;			// name of opcode
  char* arg;			// argument pattern pointer
  expressionS expr_tbl[4];	// expressions parsed
  ...
  // copy operands into buffer with whitespace stripped
  ...
  if ((opcode = (struct hc16_opcode*) hash_find (op_hash, line)) == NULL)
  {
  ...
  for (name = opcode->name; name == opcode->name; opcode++)
  {
	...
	case 'k':			// 1-byte signed constant
	  if (!parse_signbyte(&ap, &exp_p))
	    win = 0;
	  break;

	case 'f':			// 1-byte unsigned constant
	  if (!parse_absbyte(&ap, &exp_p))
	    win = 0;
	  break;

	case 'i':			// 1-byte immediate relocatable
	case 'g':			// 2-byte offset from x or y or z
	case 'h':			// 2-byte extended address
	case 'j':			// 2-byte immediate operand
	case 'r':			// 1-byte pc-relative offset
	case 'l':			// 2-byte pc-relative offset
	case 'm':			// 20 bits extended address
	case 'd':			// 20 bits offset from x or y or z
	case 's':			// swap offset and operand
	case 'q':			// for brset, brclr  
	  if (!parse_relative(&ap, &exp_p))
	    win = 0;
	  break;

	case 'o':                       // pshm multiple register mask
	  if (!parse_maskS(&ap, &exp_p))
	    win = 0;
	  break;
	case 'p':                       // pulm multiple register mask for
	  if (!parse_maskU(&ap, &exp_p))
	    win = 0;
	  break;                        // push pull instruction

	case 'a':                       // immediate 8 high 4 bits for mac
	  if (!parse_absbyte(&ap, &exp_p) || exp_p->X_add_number > 0x0F)
	    win = 0;
	  break;

	case 'b':                       // immediate 8 low 4 bits for mac
	  if (!parse_absbyte(&ap, &exp_p) || exp_p->X_add_number > 0x0F)
	    win = 0;
	  break;
	default:
    ...
    if (*arg || *ap)		// ran out of input arguments?
      win = 0;
  ...
  for (arg = opcode->args; *arg; ++arg)
  {
    ...
    if(opcode->swap)
    {
	  ...
	  case 'h':
	    sw = c;
	    for(name = opcode->name; *name; name++)
	    {
	       if (*name == 'w')
		 c = 'j';
	       else
    ...
    {
      case 'a':
	mac = exp_p->X_add_number * 0x10;
	exp_p++;
	break;
      case 'b':
	exp_p->X_add_number += mac;
	fp = frag_more(1);
	...
	fix_new_exp( frag_now,			// fragment
		     ...
		     exp_p++,			// expression to fix
		     0,				// pc-relative?
		     BFD_RELOC_20 );		// relocation type code
	...
	break;
      ...
      case 'q':
	fp = frag_more(2);
	fixP = fix_new_exp( frag_now,			// fragment
		     fp - frag_now->fr_literal,	// where in fragment
		     2,				// size in bytes
		     exp_p++,			// expression to fix
		     1,				// pc-relative?
	...
	break;
      ...
      case 'o':			// pshm multiple register
      case 'p':			// pulm multiple register
	fp = frag_more(1);
     ...
     segT seg;
{
  ...
  if ((int) fixP->fx_r_type >= (int) BFD_RELOC_UNUSED)
  {
	...
	case BFD_RELOC_16_PCREL:
	  if ((long)value < -32768 || (long)value > 32767)
	    as_bad("long branch too far");
	  md_number_to_chars (where, value, 2);
	  break;
...
/* Exactly what point is a PC-relative offset relative to?  On the hc16,
   they're relative to 6 bytes after the start of this instruction.
*/
...
)
{
  ...
  switch (fixP->fx_r_type)
  {
    ...
    case BFD_RELOC_9_PCREL:
     pcrel = fixP->fx_frag->fr_address + fixP->fx_where - 1 + 6;
     break;

    case BFD_RELOC_16_PCREL:
     if (fixP->fx_pcrel_adjust == 2)
        pcrel = fixP->fx_frag->fr_address + fixP->fx_where - 4 + 6;
             
     else        
        pcrel = fixP->fx_frag->fr_address + fixP->fx_where - 2 + 6;
...
hc1x_print_statistics (file)
     FILE *file;
{
  hash_print_statistics (file, "hc16 opcode", op_hash);
}
